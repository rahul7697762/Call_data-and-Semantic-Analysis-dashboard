...
import { Phone, MapPin, Home, DollarSign, Plus, Search, Filter } from 'lucide-react';

type ClientCall = Database['public']['Tables']['client_calls']['Row'];
...
export function ClientDashboard() {
  ...
  const filteredClients = clients.filter(client => {
    const matchesSearch =
      ...
      (client.location?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false);

    const matchesFilter =
      filterType === 'all' ||
      client.property_type === filterType;
  ...
  const stats = {
    ...
    avgBudget: clients.reduce((sum, c) => sum + (c.budget || 0), 0) / clients.length || 0,
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-2">
                ${stats.avgBudget.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-2">{stats.locations}</p>
            </div>
